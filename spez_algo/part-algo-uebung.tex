...
\While{not $u.empty$} {
    $u \gets u.pop$\;
    \For{$v \in V\ mit\ (v,w)\in E$}{
        \eIf{$side[w] = -1$}{
            $u.append(w)$\;
            $side[w] \gets 1- side[w]$ //Seite wechseln\;
        }{
            \uIf{$side[w] = side[w]$ }{
                //Konflikt: Aufsammeln \;
                $L.push(v,w)$
...
\paragraph{4)} Entfernen der Kanten L, dann im bipartiten Graphen das Matching bestimmen und dann L wieder hinzufügen.
